...
countries <- c("Canada","France", "Germany", "Italy","Japan","United Kingdom",
...
# background theme for the map

```{r}
thm3 <- hc_theme(colors = c('red', 'green', 'cyan', 'deeppink', 'yellow', 'indigo', 'blue'), 
                 chart = list(backgroundColor = "black"), 
...
hc_3 <- highchart() %>%
  hc_add_series_map(worldgeojson, gdp, value = "Value", joinBy = c('name', "CountryName"))  %>% 
  hc_colorAxis(stops = color_stops(8,c("yellow","green")), max = 120000) %>%
